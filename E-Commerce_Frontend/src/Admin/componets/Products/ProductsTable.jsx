...
const ProductsTable = () => {
  //Récupère l'objet de localisation qui contient des informations sur l'URL actuelle.
  const location = useLocation();
  //Permet de naviguer vers d'autres routes.
  const navigate = useNavigate();
  //Permet de dispatcher des actions vers le store Redux.
  const dispatch = useDispatch();
  //Sélecteur pour accéder aux parties du store Redux. Ici, il extrait customersProduct du store.
  const { customersProduct } = useSelector((store) => store);
  //Initialise l'état local filterValue avec des valeurs par défaut pour les filtres de disponibilité, de catégorie et de tri.
  ...
  //location.search : Contient la partie de la chaîne de requête de l'URL (tout ce qui suit le ?)
  const searchParams = new URLSearchParams(location.search);
  //searchParams.get("availability") : Extrait la valeur du paramètre availability
